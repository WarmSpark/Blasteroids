...
void FireBullet(Bullet bullets[], Vector2 pos, float rotation) {
  for (int i = 0; i < MAX_BULLETS; i++) {
    if (!bullets[i].active) {
      ...
      bullets[i].position.x = pos.x + sinf(rad) * 20.0f;
      bullets[i].position.y = pos.y - cosf(rad) * 20.0f;
  ...
  for (int i = 0; i < MAX_BULLETS; i++) {
    ...
    if (bullets[i].active) {
      ...
      if (bullets[i].position.x > SCREEN_WIDTH || bullets[i].position.x < 0 ||
          bullets[i].position.y > SCREEN_HEIGHT || bullets[i].position.y < 0) {
        bullets[i].active = false;
      }
    }
  }
...
void DrawBullets(Bullet bullets[]) {
  for (int i = 0; i < MAX_BULLETS; i++) {
    if (bullets[i].active) {
      DrawCircleV(bullets[i].position, 5.0f, Fade((Color){255, 50, 80, 255}, 0.35f));
      DrawCircleV(bullets[i].position, 2.8f, (Color){255, 60, 90, 255});
      DrawCircleV(bullets[i].position, 1.2f, WHITE);
    }
  }
}