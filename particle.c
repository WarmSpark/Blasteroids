...
void SpawnExhaust(Particle particle[], Vector2 position, float rotation) {
  for (int i = 0; i < MAX_PARTICLES; i++) {
    if (!particle[i].active) {
      particle[i].active = true;
      particle[i].position = position;
      float spread = (float)GetRandomValue(-25, 25) * DEG2RAD;
      float angle = (rotation * DEG2RAD) + PI + spread;
      float speed = (float)GetRandomValue(80, 160);
      particle[i].velocity.x = sinf(angle) * speed;
      particle[i].velocity.y = -cosf(angle) * speed;
      particle[i].lifetime = 0.25f;
      particle[i].maxLifetime = 0.25f;

      int c = GetRandomValue(0, 2);
      if (c == 0)
        particle[i].color = ORANGE;
      else if (c == 1)
        particle[i].color = GOLD;
      else
        particle[i].color = YELLOW;
      break;
    }
  }
}
void DrawParticle(Particle particle[]) {
  for (int i = 0; i < MAX_PARTICLES; i++) {
    if (particle[i].active) {
      ...
      DrawCircleV(particle[i].position, 2.5, Fade(particle[i].color, alpha));
    }
  }
}
